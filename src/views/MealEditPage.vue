...
const removeImageUpdateImage = async (id) => {
  const res = mealStore.deleteImageAction(route.params.id, id);
...
const updateHandler = async () => {
  ...
  try {
    const response = await mealStore.updateAction(frmData, route.params.id);
    formData.value = {
      id: "",
      name: "",
      restaurant_id: null,
...
const getDetail = async (id) => {
  const response = await mealStore.getDetailAction(id);
  ...
  formData.value.extra_price = data.extra_price ? data.extra_price : "";
      ...
      <div>
        ...
        >
          ...
          <div class="space-y-2 mt-0 sm:mt-1">
            <p class="text-sm text-gray-800">Restaurant</p>
            <v-select
