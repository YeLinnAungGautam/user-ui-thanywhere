...
const createPage = () => {
  router.push({ name: "grouptour-create" });
};
...
const changePage = async (url) => {
  ...
  // };
  await grouptourStore.getChangePage(url, watchSystem.value);
...
watch(search, async (newValue) => {
  await grouptourStore.getListAction(watchSystem.value);
});
  ...
  <div class="bg-white">
    ...
    <div class="py-5 px-4 space-y-4">
      ...
      <div
        ...
        v-if="!loading"
      >
        <div v-for="(grouptour, index) in grouptours?.data" :key="index">
          <GrouptourListItem
            :id="grouptour.id"
